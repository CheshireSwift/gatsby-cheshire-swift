...
const [bornCounts, surviveCounts] = startingRule.split('/').map(fragment =>
  fragment
    .split('')
    .map(i => parseInt(i, 10))
...
export class Board extends React.Component<{}, BoardState> {
  ...
  render() {
    const inputField = (
      <input
        ...
        type="text"
        onChange={evt => this.handleRuleChange(evt)}
      />
    );
    ...
    const spacer = <div className={css({ height: 10 })} />;
    ...
    return (
      <div>
        ...
        {statusMessage}
        <TimeControl
          onClickHandler={this.alternateAnimation.bind(this)}
  ...
  statusMessage = (rule: string) =>
    this.isValidRule(rule) ? '' : 'Not a valid rule'

  clearBoard = () =>
    this.setState({ squares: [...this.state.squares].fill(cell.state.DEAD) })
  ...
  setGenerationFunction(rule: string) {
    ...
    const newBornCounts = match[1].split('').map(i => parseInt(i, 10));
    const newSurviveCounts = match[2].split('').map(i => parseInt(i, 10));
    console.log(
      `New rule issued for born ${newBornCounts} and survive ${newSurviveCounts}`,
    );
    const generationFunction = life.computeNextGeneration(
      newBornCounts,
      newSurviveCounts,
    );
  ...
  toggleCellState(i: number) {
    ...
    squares[i] =
      squares[i] === cell.state.ALIVE ? cell.state.DEAD : cell.state.ALIVE;
